...
# Função para buscar transações e inserir no Excel
def fetch_sales_transactions(access_token, start_date, end_date, company_numbers, worksheet):
    ...
    # Linha inicial do Excel (2 para começar abaixo do cabeçalho)
    row_number = 2
        ...
        while True:
            ...
            if response.status_code == 200:
                ...
                if 'content' in data and 'transactions' in data['content']:
                    ...
                    # Processar e inserir transações no Excel
                    for transaction in transactions:
                        filtered_data = process_transaction(transaction)

                        # Escrever dados na próxima linha do Excel
                        write_transaction_to_excel(worksheet, filtered_data, row_number)
                        row_number += 1

                    # Verificar se há mais páginas de transações para buscar
...
# Função para processar os dados da transação
...
# Função para escrever a transação no Excel
...
if access_token:
    ...
    # Criar o arquivo Excel e adicionar cabeçalhos
    ...
    # Buscar as transações e salvar no Excel
    fetch_sales_transactions(access_token, start_date, end_date, companyNumbers, sheet)
